            ...
            const renderDiagram = async () => {
                try {
                    ...
                    if (mermaidRef.current) {
                        // SVG from mermaid is safe — it's generated by the library, not user input
                        mermaidRef.current.innerHTML = svg
                    }
                } catch (err: unknown) {
                    const errMsg = err instanceof Error ? err.message : "Unknown syntax error"
                    console.error("Mermaid parsing error:", errMsg)
                    if (mermaidRef.current) {
                        // ⚠ V2 FIX: Use safe DOM construction — never inject raw error strings via innerHTML
                        mermaidRef.current.innerHTML = ""
                        const wrapper = document.createElement("div")
                        wrapper.className = "flex flex-col items-center gap-3 p-6 text-center"

                        const icon = document.createElement("p")
                        icon.className = "font-mono text-2xl"
                        icon.textContent = "⚠"
                        wrapper.appendChild(icon)

                        const title = document.createElement("p")
                        title.className = "font-mono text-xs font-bold uppercase tracking-widest text-rose-400"
                        title.textContent = "Diagram Syntax Error"
                        wrapper.appendChild(title)

                        const detail = document.createElement("pre")
                        detail.className = "font-mono text-[10px] text-muted-foreground whitespace-pre-wrap text-left max-w-sm"
                        detail.textContent = errMsg  // textContent is always safe
